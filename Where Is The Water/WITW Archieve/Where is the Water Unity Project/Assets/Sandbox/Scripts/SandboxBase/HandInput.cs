...
namespace ARSandbox
{
    public class HandInput : MonoBehaviour
    {
        ...
        public bool OnUITouchDown(int touchID, Vector2 screenSpacePoint)
        {
            Vector3 notCirclePosition = Vector3.zero;
            if (!CurrentGestures.Exists((gesture) => gesture.GestureID == touchID))
            {
                ...
                // If the ID matches the circle tracking script, create water here.
                //if(touchID == 7045)
                //{
                    //return worldPosition;
                    //_WaterSimulation.CreateWaterFromCircle(worldPosition);
                //}
                return true;
            }
            else
            ...
            //return notCirclePosition;
        }
        public bool OnUITouchMove(int touchID, Vector2 screenSpacePoint)
        {
        ...
        public bool OnUITouchUp(int touchID)
        {
            HandInputGesture UIGesture = CurrentGestures.Find((gesture) => gesture.GestureID == touchID);
            if (UIGesture != null)
            {
